...
如果你不想自己安装和配置，可以直接使用云厂商提供的 OpenClaw 托管服务。例如火山引擎的 [ArkClaw](https://www.volcengine.com/product/arkclaw) 提供了开箱即用的云端 OpenClaw，无需安装 Node.js、无需配置 API Key，注册后即可使用。适合想快速体验 OpenClaw 能力、不想折腾本地环境的用户。

> 本教程以本地安装为主线，因为这能让你完全掌控 OpenClaw 的配置和数据。云端托管的操作方式类似，学会本地使用后可以无缝迁移。
